...
function MisContratosCliente() {
    ...
    useEffect(() => { 
        ContratoServiceInstance.getProveedores(cliente.idCliente)
            .then((contratos) => {
    ...
    const filtrarContratos = (contratos) => {
        ...
        return contratos.filter((contrato) => {
            const nombreCliente = `${contrato.idProveedor.nombre} ${contrato.idProveedor.apellidoPaterno} ${contrato.idProveedor.apellidoMaterno}`.toLowerCase();
            ...
            <div className='border rounded-lg w-full py-6 px-8 grid grid-cols-2 lg:grid-cols-3 gap-4'>
                <div className='flex flex-col px-4'>
                    <div className='flex flex-wrap justify-start'>
                        <img className='className="w-20 h-20 rounded-full object-cover border-gray-300 border' src={contrato.idProveedor.imagenUrl} alt='Foto del cliente'/>
...
export default MisContratosCliente
