...
function Chats() {
  ...
  const [isEdit, setIsEdit] = useState(false); // Nuevo estado para distinguir entre crear y editar
  ...
  const getServicios = () => {
    ServiciosService.getLista()
      .then((ServicioResponse) => {
        ...
        setFilteredServicios(ServicioResponse); // Inicializa los servicios filtrados con todos los servicios
      })
      .catch((error) => {
        console.log(error);
      });
